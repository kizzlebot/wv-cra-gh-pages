 ...
 */
export const initRectAnnot = (name, customData = {}) => async ({ instance, tools, header, ...rest }) => {
    ...
    const C = ({
      [className]: class extends ClassDef {
        ...
        setSigner(id) {
          ...
          this.setContents(`${customData.label ? customData.label : name}: ${fullName}`);
  ...
  const createFreeText = async (rectAnnot, custom = {}) => {
    ...
    textAnnot.setContents(`${custom.label ? custom.label : name}: ${toFullName(textAnnot.custom.name)}`);
  ...
  const createToolClass = (className, RectAnnotClass) => {
    const C = class extends Tools.GenericAnnotationCreateTool {
      constructor(docViewer) {
        ...
        this.on('annotationAdded', (annot) => createFreeText(annot, customData));
      }
    };
...
export const initApplySigCreator = (name) => async ({ instance, tools, header, ...rest }) => {
  ...
  const applySigConvert = async (tool) => {
    ...
    let annotsToDraw = [];
    const annotsToDelete = await Promise.map(annotationsList, async (annot, index) => {
      let field;
      let inputAnnot;
    ...
    _.map(annotsToDraw, () => tool.trigger('annotationAdded', annotsToDraw));
  ...
  class ApplySigFieldTool extends Tools.Tool {
    ...
    async switchIn(oldTool) {
      this.on('annotationAdded', (annot) => {
        console.log('signaturewidgetadded', annot)
      })
      await applySigConvert(this);
      setTimeout(() => {
