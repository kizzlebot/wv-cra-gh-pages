...
((exports) => {
  ...
  */
      ...
      if (annotType === 'annotation') {
        ...
        annotations.forEach(async annotation => {
          ...
          if (author !== authorId) {
            ...
            await annotation.updateReadOnly(
              this.props.userType,
              this.state.selectedSigner,
              this.state.locked
            );
            return;
          }
  ...
  window.addEventListener('viewerLoaded', async () => {
    ...
    annotManager.on('setSigner', async (val) => {
      console.log('setSigner called', val);
      signer = val;
      return annotManager.setCurrentUser(val?.id || 'guest')
    });
